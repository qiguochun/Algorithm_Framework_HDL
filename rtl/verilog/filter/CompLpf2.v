...
//Moudle Name       :   CompLpf2.v
//Original Author   :   HDL-Auto
//Creation Date     :   2026.09.06
/*Description       :   二阶巴特沃斯低通滤波器（双线性/Tustin 离散，IIR）。
                        连续 H(s)=wc^2/(s^2+sqrt(2)*wc*s+wc^2)，wc=2*pi*fc。
                        s=2fs*(z-1)/(z+1) 直接代入，记 K=2fs, C=wc^2+sqrt2*wc*K+K^2：
                        分子 N=wc^2(z+1)^2 = wc^2*(1+2z^-1+z^-2)；
                        ...
                        状态量化一拍、饱和一拍、输出量化/状态更新一拍，共 LATENCY=6 拍；
                        iEn 单拍脉冲采样一帧，完成后 oValid 拉高一拍。
