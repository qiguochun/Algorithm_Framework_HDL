...
//Moudle Name       :   CompBpf2.v
//Original Author   :   HDL-Auto
//Creation Date     :   2026.09.06
/*Description       :   二阶巴特沃斯带通滤波器（双线性/Tustin 离散，IIR）。
                        连续 H(s)=(wc/Q)*s/(s^2+(wc/Q)*s+wc^2)，wc=2*pi*f0，Q=品质因数。
                        s=K*(z-1)/(z+1)，K=2fs，记 C=K^2+(wc/Q)K+wc^2：
                        分子 =(wc/Q)K*(z^2-1) -> b0=(wc/Q)K/C, b1=0, b2=-b0；
                        分母 z^0..z^2 系数 = C,(2wc^2-2K^2),(K^2-(wc/Q)K+wc^2) ->
                        a1=(2wc^2-2K^2)/C, a2=(K^2-(wc/Q)K+wc^2)/C。
                        差分 y[n]=b0*x[n]+b1*x[n-1]+b2*x[n-2]-a1*y[n-1]-a2*y[n-2]。
                        f0/fs/Q 隐含于 Q 系数 localparam（默认 f0=250Hz, fs=5kHz, Q=0.707，
                        Python 推导）。中心增益=1，直流/高频阻塞。
...
module CompBpf2 #(
    ...
    parameter integer F0         = 250,    // 中心频率 Hz（文献/顶层参考，wc=2*pi*F0）
    ...
    parameter integer COEF_B0    = 186825,   // b0=(wc/Q)K/C    =0.17817025 *2^20  Q20
    parameter integer COEF_B1    = 0,        // b1=0
    parameter integer COEF_B2    = -186825,  // b2=-(wc/Q)K/C(=-b0)            Q20
    parameter integer COEF_A1    = -1640499, // a1=(2wc^2-2K^2)/C=-1.56450131 *2^20 Q20
    parameter integer COEF_A2    = 674926    // a2=(K^2-(wc/Q)K+wc^2)/C=0.64365951 *2^20 Q20
) (
    input  wire                iSysClk,  // 时钟
    input  wire                iSysRst,  // 复位（高有效）
    input  wire                iEn,      // 采样节拍（单拍脉冲：一帧一个采样）
    input  wire signed [W-1:0] iXIn,     // 输入采样 x[n]
    output reg  signed [W-1:0] oYOut,    // 带通输出 y[n]
